 ...
 */
Route::get('/', function () {
// return view('email.email-verification')->with(['token' => '34344']);
...
if (!$is_published) {
    Route::group(['prefix' => 'payment'], function () {
        ...
        Route::group(['prefix' => 'paystack', 'as' => 'paystack.'], function () {
            ...
            Route::post('webhook', [PaystackController::class, 'validateWebhook'])->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class]);
        });

        //Monnify
        Route::group(['prefix' => 'monnify', 'as' => 'monnify.'], function () {
            Route::get('pay', [MonnifyController::class, 'index'])->name('pay');
            // Route::post('payment', [MonnifyController::class, 'redirectToGateway'])->name('payment');
            Route::get('callback', [MonnifyController::class, 'handleGatewayCallback'])->name('callback');
            Route::post('webhook', [MonnifyController::class, 'validateWebhook'])->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class]);
        });
        ...
        //PAGE
        Route::group(['prefix' => 'paga', 'as' => 'paga.'], function () {
