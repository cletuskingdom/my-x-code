...
trait Payment
{
    public static function generate_link(object $payer, object $payment_info, Object $receiver, $remove_cutlery=null)
    {
        ...
        $payment->reference = Str::random(30);
        $payment->save();

        // if($payment->order_id != null){
        //     $order = Order::where('id', $payment->order_d)->first();
        // }
        ...
        if (array_key_exists($payment->payment_method, $routes)) {
            return  $remove_cutlery === null ?
            url("{$routes[$payment->payment_method]}/?payment_id={$payment->id}&&order_id={$payment_info->order_id}") : 
            url("{$routes[$payment->payment_method]}/?payment_id={$payment->id}&&order_id={$payment_info->order_id}&&remove_cutlery={$remove_cutlery}");
