...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request): JsonResponse
    {
        ...
        if ($request->is_partial == 1) {
            $payment_status = ($request->payment_method == 'cash_on_delivery' || $request->payment_method == 'offline_payment') ? 'partial_paid' : ($request->payment_method == 'wallet_payment' ? 'paid' : 'unpaid');
        } else {
            $payment_status = ($request->payment_method == 'cash_on_delivery' || $request->payment_method == 'offline_payment') ? 'unpaid' : ($request->payment_method == 'wallet_payment' ? 'paid' : 'unpaid');
        }

        $order_status = ($request->payment_method == 'cash_on_delivery' || $request->payment_method == 'offline_payment') ? 'pending' : ($request->payment_method == 'wallet_payment' ? 'confirmed' : 'pending');
        $confirmed_at = ($request->payment_method == 'cash_on_delivery' || $request->payment_method == 'offline_payment') ? null : ($request->payment_method == 'wallet_payment' ? now() : null);
        ...
        try {
            ...
            $or = [
                ...
                'kitchen_note' => ($request['kitchen_note'] === 'empty') ? null : $request['kitchen_note'],
                ...
                'created_at' => now(),
                'updated_at' => now(),
                'remove_cutlery' => $request->has('remove_cutlery') ? $request['remove_cutlery'] : false,
                // new addition
            ...
            try {
                if ($value && isset($fcm_token) && $request->payment_method != 'offline_payment') {
                    $data = [
                    ...
                    Helpers::send_push_notif_to_specific_user($fcm_token, 'Order placed', 'Excellent pick, Confirmation Complete!');
                } else {
                    $data = [
                        'title' => translate('Order'),
                        'description' => $value,
                    ...
                    Helpers::send_push_notif_to_specific_user($fcm_token, 'Order placed', 'Your order will be confirmed soon!');
                }
            } catch (\Exception $e) {
                //
            }
            ...
            $data = [
                'title' => 'You have a new order - (' . $or['order_status'] . ')',
                'description' => $order_id,
                'order_id' => $order_id,
                'image' => '',
            ];
            $dataNew = [
                'title' => 'You have a new order (First Time Order) - (' . $or['order_status'] . ')',
                'description' => $order_id,
                'order_id' => $order_id,
                'image' => '',
            ];
            ...
            try {
                // check if the user is a first tiem customer
                $response = Helpers::notifyKitchenFirstTimeCustomer($user_id);

                // Handle the response
                if ($response['status'] == 'success') {
                    // first time
                    Helpers::send_push_notif_to_topic($dataNew, "kitchen-{$or['branch_id']}", 'general');
                    Helpers::send_push_notif_to_topic_new(
                        "kitchen-{$or['branch_id']}",
                        'New Order(First Time)',
                        'You have a new order for a FIRST TIME USER - (' . $or['order_status'] . ')'
                    );
                } else {
                    // not his first time
                    ...
                    Helpers::send_push_notif_to_topic_new(
                        "kitchen-{$or['branch_id']}",
                        'New Order',
                        'You have a new order - (' . $or['order_status'] . ')'
                    );
                }
            ...
            // }
            $has_previous_orders = Order::where('user_id', $user_id)
                ->where('id', '!=', $order_id)  // Ensure the current order isn't counted
                ->exists();

            // If no previous orders, set $is_first_order to 1, otherwise set it to 0
            $is_first_order = $has_previous_orders ? 0 : 1;

            // Update the order record with the correct value
    ...
    {
        // Retrieve the authenticated user's ID (instead of hardcoding user_id = 13)
        ...
        // Retrieve the user's orders where 'checked' is 0
        $orders = Order::where('user_id', $userId)->where('checked', 0)->get();

        // Check if the user has placed any previous orders
        $hasOrderedBefore = Order::where('user_id', $userId)->where('checked', 1)->exists();

        // If the user has no previous orders (first-time customer)
        if (!$hasOrderedBefore) {
            return response()->json([
                'error' => "Sorry, you can't proceed with the request since you are a first-time user"
            ], 401);
        }

        // If the user is not a first-time customer, return their last order
        $lastOrder = $this->order->where('user_id', $userId)->latest()->first();
