...
class ProductController extends Controller
{
    ...
    public function index(): Renderable
    {
        ...
        $groups = ProductGrouping::all();
        return view('admin-views.product.index', compact('categories', 'groups'));
    }
    ...
    public function list(Request $request): Renderable
    {
        ...
        // Add total_sales calculation and order by total_sales
        $products = $query->with('main_branch_product')
            ->withSum(['orderDetails as total_sales' => function ($query) {
                $query->whereHas('order', function ($q) {
                    $q->where('order_status', 'delivered');
                });
            }], 'quantity') // Sum the 'quantity' field from OrderDetail
        ...
        // Return the view with the products and search parameters
        return view('admin-views.product.list', compact('products', 'search', 'category_name', 'category_list', 'categoryId'));
    ...
    public function store(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            // 'image' => 'required|mimes:jpeg,jpg,svg,png|max:500',
        ...
        $product->price =  Helpers::sanitize_input($request->price);
        $product->set_menu =  Helpers::sanitize_input($request->item_type);
        $product->product_type =  Helpers::sanitize_input($request->product_type);
        $product->image = Helpers::upload('product/', 'png', $request->file('image'));
        $product->available_time_starts =  Helpers::sanitize_input($request->available_time_starts);
        $product->available_time_ends =  Helpers::sanitize_input($request->available_time_ends);

        $product->tax = $request->tax_type == 'amount' ? Helpers::sanitize_input($request->tax) : Helpers::sanitize_input($request->tax);
        $product->tax_type =  Helpers::sanitize_input($request->tax_type);

        $product->discount = $request->discount_type == 'amount' ?  Helpers::sanitize_input($request->discount) :  Helpers::sanitize_input($request->discount);
        $product->discount_type =  Helpers::sanitize_input($request->discount_type);
        ...
        $product->grouping_id = Helpers::sanitize_input($request->grouping);
        $product->save();
        ...
        $main_branch_product->price =  Helpers::sanitize_input($request->price);
        $main_branch_product->discount_type =  Helpers::sanitize_input($request->discount_type);
        $main_branch_product->discount =  Helpers::sanitize_input($request->discount);
        $main_branch_product->branch_id = 1;
        $main_branch_product->is_available = 1;
        $main_branch_product->variations = $variations;
        $main_branch_product->stock_type =  Helpers::sanitize_input($request->stock_type);
        $main_branch_product->stock =  Helpers::sanitize_input($request->product_stock) ?? 0;
    ...
    public function edit($id): Renderable
    {
        ...
        $categories_product = $this->product->whereJsonContains('category_ids', ['id' => $positions[0]['id']])->get();
        $count_ = 1;
        $count_2 = 1;
        $count_3 = 1;

        return view('admin-views.product.edit', compact('product', 'product_category', 'categories', 'groups', 'positions', 'position', 'categories_product', 'count_', 'count_2', 'count_3'));
    }
    ...
    public function update_position(Request $request): JsonResponse
    {
    ...
    {   
        ...
        // Extract the `position` for the matching `category_id`
        $products = $query
            ->whereJsonContains('category_ids', [['id' => (string)$categoryId]])
            ->with('main_branch_product')
            ->withSum(['orderDetails as total_sales' => function ($query) {
                $query->whereHas('order', function ($q) {
                    $q->where('order_status', 'delivered');
                });
            ...
            ->select('*', DB::raw("
                JSON_UNQUOTE(
                    JSON_EXTRACT(
                        category_ids, 
                        '$[0].position'  -- Adjust based on how your data is stored
                    )
                ) AS extracted_position
            "))
            ->orderBy('extracted_position', 'asc')  // Sort by extracted position
            ->paginate(Helpers::getPagination())
            ->appends($query_param);

        $category_name = Category::whereId($categoryId)->value("name");
        $category_list = Category::latest()->get();
