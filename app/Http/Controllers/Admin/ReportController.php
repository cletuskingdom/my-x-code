...
class ReportController extends Controller
{
    ...
    public function sale_filter(Request $request): JsonResponse
    {
        ...
        $orders = $this->order->whereBetween('created_at', [$fromDate, $toDate]);
        ...
        foreach ($this->order_detail->whereIn('order_id', $orders)->with('transaction')->latest()->get() as $detail) {
            $trans = $detail['transaction']? $detail['transaction']['delivery_charge']:0.00;
            // get the data type of $trans

            // dd($trans['delivery_charge']);
            // dd(gettype(($trans)));
        ...
        return response()->json([
            ...
            'delivery_sum' => Helpers::set_symbol($total_delivery),
            'view' => view('admin-views.report.partials._table', compact('data'))->render(),
        ]);
    }
