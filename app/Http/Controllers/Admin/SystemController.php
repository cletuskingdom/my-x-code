...
class SystemController extends Controller
{
    ...
    public function restaurant_data_new(): JsonResponse
    {
        // Retrieve all orders where 'checked' is 0
        $orders = $this->order->where(['checked' => 0])->get();
        $hasFirstTimeCustomer = false; // Flag to track if there is any first-time customer

        foreach ($orders as $order) {
            // Check if the user has any previous orders other than the current one
            $hasOrderedBefore = Order::where('user_id', $order->user_id)
                // Exclude the current order
                ->where('id', '!=', $order->id)
            ->exists();

            // If this is a first-time customer, set the flag
            if (!$hasOrderedBefore) {
                $hasFirstTimeCustomer = true;
                break; // No need to check further once we find a first-time customer
            }
        }

        // Return the orders and the first_time_customer flag
        return response()->json([
            'success' => 1,
            'first_time_customer' => $hasFirstTimeCustomer ? 1 : 0,
            'data' => ['new_order' => $orders->count()]
        ]);
    }
