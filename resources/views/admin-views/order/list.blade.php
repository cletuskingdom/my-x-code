                ...
                <div class="table-responsive datatable-custom">
                    <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                        ...
                        @foreach($orders as $key=>$order)
                            <tr class="status-{{$order['order_status']}} class-all" id="{{'order-'.$order['id']}}">
                                ...
                                <td >
                                    <div>{{ \App\CentralLogics\Helpers::set_symbol($order['order_amount']-$order['coupon_discount_amount']) }}</div>
                                     ...
                                     <div class="payment-status">
                                        ...
                                        @elseif($order->payment_status=='unpaid')
                                            <span class="text-secondary">{{translate('unpaid')}}</span>
                                ...
                                <td class="text-capitalize status-data">
                                ...
                                <td>{{$order['remove_cutlery'] == 0 ? 'Yes' : 'No'}}</td>
                                <td>
...
@push('script_2')
    ...
    <script>
        ...
        if(page !== null && parseInt(page) > 1){
            url = "{{route('admin.orders.list', 'all').'?api=true'}}&&page="+page
        }else{
            url = "{{route('admin.orders.list', 'all').'?api=true'}}"
        }
        const firstOrderIdOnPage = document.querySelector('.class-all').id.split('-')[1];
        ...
        setInterval(() => {
                ...
                success:function(data){
                    ...
                    console.log(data);
                    if(data?.orders?.data && data?.orders?.data[0]?.id != firstOrderIdOnPage){
                        location.reload();
                    }else{
                        const status_colors = {
                            ...
                            'out_for_delivery' : 'badge-soft-warning',
                        ...
                        let affected_row = document.querySelector(`#order-${order.id}`)
                        affected_row.querySelector('.status-data').innerHTML = `<span class="${status_colors[order.order_status]} px-2 py-1 rounded">${order.order_status.replaceAll('_',' ')}</span>`;

                        affected_row.querySelector('.payment-status').innerHTML = `<span class="${payment_status[order.payment_status]}">${order.payment_status}</span>`;
                        });
                    }
