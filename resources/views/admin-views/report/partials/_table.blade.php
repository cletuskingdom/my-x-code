    ...
    <tbody>
        @foreach($data as $key=>$row)
            <tr>
                ...
                <td>{{ \Str::headline($row['order_type']) }}</td>

                {{-- <td>{{ \App\CentralLogics\Helpers::set_symbol($row['price']) }}</td>
                <td>{{ \App\CentralLogics\Helpers::set_symbol($row['delivery_fee'])}}</td>
                <td>{{ \App\CentralLogics\Helpers::set_symbol($row['delivery_charge']) }}</td> --}}
            </tr>
        @endforeach
    </tbody>
</table>
